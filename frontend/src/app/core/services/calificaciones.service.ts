...
export class CalificacionesService {
   ...
   */
  abrirModalCalificacion(viaje: Viaje, onSuccess?: () => void): void {
    const conductor = viaje.conductor;
    if (!conductor) return;
    this._mostrarFormCalificacion(viaje, conductor, onSuccess);
  }

  private _mostrarFormCalificacion(
    viaje: Viaje,
    conductor: { id: number; nombre: string; apellido: string; foto_perfil?: string },
    onSuccess?: () => void,
  ): void {
    const avatar = conductor.foto_perfil
      ? `<img src="${conductor.foto_perfil}" style="width:56px;height:56px;border-radius:50%;object-fit:cover;border:2px solid #e5e7eb">`
      : `<div style="width:56px;height:56px;border-radius:50%;background:#e5e7eb;display:flex;align-items:center;justify-content:center;font-size:1.5rem;font-weight:700;color:#6b7280">${conductor.nombre.charAt(0).toUpperCase()}</div>`;
    ...
    Swal.fire({
      ...
      html: `
        <div style="display:flex;flex-direction:column;align-items:center;gap:8px;margin-bottom:12px">
          ${avatar}
          <p style="font-weight:600;color:#1f2937;margin:0">${conductor.nombre} ${conductor.apellido}</p>
          <p style="font-size:0.75rem;color:#6b7280;margin:0">Conductor · Viaje #${viaje.id}</p>
        </div>
